...
int senior_disp(void)
{
    ...
    printf("\n --------------------");
    count = 0;
    for (kaiin_code = 1; kaiin_code <= MEMBER_MAX; kaiin_code++) {
        if (codedata_tbl[kaiin_code - 1] != 0) {
            if (kojin_data_read(kaiin_code) == NG)
                return NG;
            if (kojin_keisoku_tbl.age >= 60) {
                printf("\n %03d : %2d歳", kaiin_code, kojin_keisoku_tbl.age);
                count++;
            }
        }
    }
