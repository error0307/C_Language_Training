...
#ifdef TOI1
        /* 会員計測情報保存 */
        if ((ret = kojin_data_save(kaiin_code)) == NG) {
            /* コード・データ対照表ファイル CLOSE */
            fclose(fp);
            return ret;
        }
...
{
    FILE    *fp;                   /* 個人計測データ保存ファイルポインタ  */
    char    fname[8];              /* 個人計測データ保存ファイル名        */
    int     i;                     /* インデックス                        */

    /* 個人計測データ保存ファイル OPEN -> NULL ? */
    sprintf(fname, "%03d.dat", kaiin_code);
    if ((fp = fopen(fname, "wt")) == NULL) {
        printf("\n 個人計測データ保存ファイル OPEN エラー");
        return NG;
    }

    fprintf(fp, "会員コード　　　: %d\n", kojin_keisoku_tbl.kaiin_code);
    fprintf(fp, "計測回数　　　　: %d\n", kojin_keisoku_tbl.count);
    if (kojin_keisoku_tbl.count >= 1) {
        fprintf(fp, "初回日付　　　　: %4.4s-%2.2s-%2.2s\n",
            kojin_keisoku_tbl.first_date + 0,
            kojin_keisoku_tbl.first_date + 4,
            kojin_keisoku_tbl.first_date + 6);
        fprintf(fp, "初回データ　　　: %d\n", kojin_keisoku_tbl.first_data);
        fprintf(fp, "最高記録日付  　: %4.4s-%2.2s-%2.2s\n",
            kojin_keisoku_tbl.max_date + 0,
            kojin_keisoku_tbl.max_date + 4,
            kojin_keisoku_tbl.max_date + 6);
        fprintf(fp, "最高記録データ　: %d\n", kojin_keisoku_tbl.max_data);
        fprintf(fp, "最新計測日付　　: %4.4s-%2.2s-%2.2s\n",
            kojin_keisoku_tbl.soku_date + 0,
            kojin_keisoku_tbl.soku_date + 4,
            kojin_keisoku_tbl.soku_date + 6);
        fprintf(fp, "最新データ　　　: %d\n", kojin_keisoku_tbl.soku_data[0]);
        for (i = 1; i < 10 && i < kojin_keisoku_tbl.count; i++) 
            fprintf(fp, "%2d回前データ　　: %d\n", i, kojin_keisoku_tbl.soku_data[i]);
    }
    fclose(fp);
    return OK;
}
