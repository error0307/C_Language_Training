...
int kaiin_merge( void )
{
    int     ret;                               /* リターンコード              */
    int     i;                                 /* インデックス                */
    int     kaiin_code_a;                      /* 計測会員コードA             */
    int     kaiin_code_b;                      /* 計測会員コードB             */
    struct  KEISOKU_TBL kojin_keisoku_tbl_wk;  /* 個人別計測データ表 退避領域 */
    int     soku_data_wk[10];                  /* 計測データワーク            */
    long    fptr;                              /* 計測データポインタ          */
    FILE    *fp;                               /* ファイルポインタ            */
    char    *fname = KEISOKU_TBL_NAME;         /* 計測データ表ファイル        */
    ...
    /* 計測会員コードA入力 */
    kaiin_code_input( &kaiin_code_a );

    /* 計測会員コードB入力 */
    kaiin_code_input( &kaiin_code_b );

    /* 会員コードの一致チェック */
    if (kaiin_code_a == kaiin_code_b)
    {
        printf( "\n 会員コードが同じです" );
        return NG;
    }

    /* 計測会員コードBの計測データ表 READ -> NG ? */
    if ((ret = kojin_data_read( kaiin_code_b )) == NG) {
        return ret;
    }

    /* 計測データ表の退避 */
    kojin_keisoku_tbl_wk = kojin_keisoku_tbl;

    /* 計測会員コードAの計測データ表 READ -> NG ? */
    if ((ret = kojin_data_read( kaiin_code_a )) == NG) {
        return ret;
    }

    if (kojin_keisoku_tbl_wk.count >= 1)
    {
        if (kojin_keisoku_tbl.count == 0) {
            /* 計測会員コードBの計測情報を使う */
            kojin_keisoku_tbl = kojin_keisoku_tbl_wk;
            kojin_keisoku_tbl.kaiin_code = kaiin_code_a;
        } else {
            /* 計測会員コードAもBも計測回数１以上 */

            /* 初回日付、初回データの更新 */
            if (strcmp( kojin_keisoku_tbl.first_date, kojin_keisoku_tbl_wk.first_date ) > 0) {
                strcpy( kojin_keisoku_tbl.first_date, kojin_keisoku_tbl_wk.first_date );
                kojin_keisoku_tbl.first_data = kojin_keisoku_tbl_wk.first_data;
            }

            /* 最高記録日付、最高記録データの更新 */
            if (kojin_keisoku_tbl.max_data < kojin_keisoku_tbl_wk.max_data ) {
                strcpy( kojin_keisoku_tbl.max_date, kojin_keisoku_tbl_wk.max_date );
                kojin_keisoku_tbl.max_data = kojin_keisoku_tbl_wk.max_data;
            }

            /* 最新計測日付、計測データの更新 */
            if (strcmp( kojin_keisoku_tbl.soku_date, kojin_keisoku_tbl_wk.soku_date ) < 0) {
                strcpy( kojin_keisoku_tbl.soku_date, kojin_keisoku_tbl_wk.soku_date );

                /* 計測データB , 計測データAの順に格納 */
                for (i = 0; i < kojin_keisoku_tbl_wk.count && i < 10; i++)
                    soku_data_wk[i] = kojin_keisoku_tbl_wk.soku_data[ i ];
                for (; i < 10; i++)
                    soku_data_wk[i] = kojin_keisoku_tbl.soku_data[ i - kojin_keisoku_tbl_wk.count ];
            } else {
                /* 計測データA , 計測データBの順に格納 */
                for (i = 0; i < kojin_keisoku_tbl.count && i < 10; i++)
                    soku_data_wk[i] = kojin_keisoku_tbl.soku_data[ i ];
                for (; i < 10; i++)
                    soku_data_wk[i] = kojin_keisoku_tbl_wk.soku_data[ i - kojin_keisoku_tbl.count ];
            }
            memcpy( kojin_keisoku_tbl.soku_data, soku_data_wk, sizeof kojin_keisoku_tbl.soku_data);

            /* 計測回数の加算 */
            kojin_keisoku_tbl.count += kojin_keisoku_tbl_wk.count;
        ...
        /* 該当データポインタ セット */
        fptr = (codedata_tbl[kaiin_code_a - 1] - 1) *
            sizeof( struct KEISOKU_TBL );
        ...
        /* 計測データ表ファイル WRITE -> 1以外 ? */
        if ((ret = fwrite( (char *)&kojin_keisoku_tbl, sizeof( kojin_keisoku_tbl ), 1, fp )) != 1) {
            printf( "\n 計測データ表ファイル WRITE エラー" );
            ret = NG;
        }
        else {
    ...
    /* 計測結果データ表示 */
    kojin_data_disp( kaiin_code_a, "\n ** 計測データマージ結果 **" );

    printf( "\n 計測会員 [ %d ] に 計測会員 [ %d ] のデータをマージしました", kaiin_code_a, kaiin_code_b );
    printf( "\n 登録削除を実行し，計測会員 [ %d ] は削除してください", kaiin_code_b );
