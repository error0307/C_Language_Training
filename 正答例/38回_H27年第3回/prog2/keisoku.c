...
int kesseki_kaiin_disp( void )
{
    ...
    FILE    *fp;                        /* ファイルポインタ       */
    char    *fname = KEISOKU_TBL_NAME;  /* 計測データ表ファイル   */
    char    current_ym[7];              /* 現在の年月             */
    char    check_ym[7];                /* 比較する年月           */
    int     year;                       /* 年                     */
    int     month;                      /* 月                     */
    ...
    /* 現在の年月入力 */
    current_ym_input( current_ym );

    /* 3か月前の年月を求める */
    sscanf(current_ym, "%04d%02d", &year, &month);
    month -= 3;
    if (month < 1) {
        year--; 
        month += 12;
    }
    sprintf(check_ym, "%04d%02d", year, month);
    ...
    for( ; ; ) {
        ...
        /* 計測データ表ファイル READ -> 1以外 ? */
        if( (ret = fread( (char *)&sort_keisoku_tbl[ cnt ],
                   sizeof( struct KEISOKU_TBL ), 1, fp )) != 1 ) {
        ...
        /* １回以上計測あり */
        if( sort_keisoku_tbl[ cnt ].count > 0 ) {
            /* 最新計測日付(年月) ≦ チェック年月 */
            if (strncmp(sort_keisoku_tbl[ cnt ].soku_date, check_ym, 6) <= 0)
                cnt++;
        }
    ...
    if( ret == OK ) {
        if ( cnt == 0 ) {
            ...
            for( i = 0; i < cnt; i++ ) {
                printf( "\n %4.4s-%2.2s-%2.2s   %3d",
                    &sort_keisoku_tbl[ i ].soku_date[ 0 ],
                    &sort_keisoku_tbl[ i ].soku_date[ 4 ],
                    &sort_keisoku_tbl[ i ].soku_date[ 6 ],
                    sort_keisoku_tbl[ i ].kaiin_code);
            }
        }
    }
...
/*   リターン   : なし              */
/************************************/
static void current_ym_input( char *current_ym )
{
    int     loop = TRUE;    /* ループフラグ  */
    int     chk_ym;         /* 年月数値      */
    char    conv[ 5 ];      /* 数値変換用    */
...
/*   リターン   : なし                      */
/********************************************/
static void keisoku_date_sort( int cnt )
{
    int     i;                      /* インデックス      */
    int     j;                      /* インデックス      */
    struct  KEISOKU_TBL  work;      /* スワップ用エリア  */
    ...
    for( i = 0; i < cnt - 1; i++ ) {
        for( j = i + 1; j < cnt; j++ ) {
            /* 計測日付が逆順か？ */
            if( strcmp(sort_keisoku_tbl[ i ].soku_date, sort_keisoku_tbl[ j ].soku_date ) > 0) {
                work = sort_keisoku_tbl[ i ];
                sort_keisoku_tbl[ i ] = sort_keisoku_tbl[ j ];
                sort_keisoku_tbl[ j ] = work;
            } else if (strcmp( sort_keisoku_tbl[ i ].soku_date, sort_keisoku_tbl[ j ].soku_date ) == 0) {
                /* 計測日付が一致していた場合、会員コードで比較 */
                if (sort_keisoku_tbl[ i ].kaiin_code > 
